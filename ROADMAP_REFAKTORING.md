...
1. `TODO` Srovnat Composer boundary mezi root skeletonem a `mipress/core`.
   ...
   Další quick fix je také hotový: root skeleton přešel na veřejná Composer jména `michalcabala/*` a už není navázaný na lokální `path` repositories, takže release model je blíž reálnému public install flow.
